...
export default function MarketDetailClientPage({ id, initialMarket }: Props) {
  const marketId = BigInt(id);
  const { data, isLoading, error, refetch } = useMarket(marketId, initialMarket);
  ...
  if (!data) {
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 py-8 text-center">
        ...
        {isRpcError && (
          <>
            <p className="text-sm text-text-muted mt-1">
              API request failed — check your connection.
            </p>
            <button
              type="button"
              onClick={() => refetch()}
        ...
        <Link
          href="/"
          className="text-text-secondary text-sm underline mt-4 block"
        >
          ← Back to markets
        </Link>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <Link
        href="/"
        className="text-text-muted hover:text-text-secondary text-sm mb-6 block transition-colors"
      >
        ← All markets
      </Link>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-surface-card border border-border rounded-xl p-5">
            <div className="flex justify-between mb-1">
              <div>
                <span className="text-yes font-bold text-2xl">
                  {formatSharePrice(yesProbability)}
                </span>
              ...
              <div className="text-right">
                <span className="text-no font-bold text-2xl">
                  {formatSharePrice(1 - yesProbability)}
                </span>
                <span className="text-no text-sm ml-2">NO</span>
                <p className="text-text-muted text-xs mt-0.5">
                  {(100 - yesProbability * 100).toFixed(1)}% probability
                </p>
              </div>
            </div>
          ...
          <div className="bg-surface-card border border-border rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="text-text-primary font-medium">
                  {formatUsdc(pool.totalCollateral)} mUSDC
                </p>
              </div>
            </div>
          </div>
